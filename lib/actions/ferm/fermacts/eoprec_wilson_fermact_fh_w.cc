...
#include "actions/ferm/fermacts/eoprec_wilson_fermact_fh_w.h"
...
namespace Chroma
{
    ...
    namespace EvenOddPrecWilsonFermActFHEnv
    {
        ...
        WilsonTypeFermAct<LatticeFermion,
                          ...
                          multi1d<LatticeColorMatrix>> *
        createFermAct4D(XMLReader &xml_in,
                        const std::string &path)
        {
            return new EvenOddPrecWilsonFermActFH(CreateFermStateEnv::reader(xml_in, path),
        ...
        //! Name to be used
        const std::string name("WILSON_FH");
   ...
   */
    EvenOddPrecConstDetLinearOperator<LatticeFermion,
                                      multi1d<LatticeColorMatrix>,
                                      multi1d<LatticeColorMatrix>> *
    EvenOddPrecWilsonFermActFH::linOp(Handle<FermState<T, P, Q>> state) const
    {
        return new EvenOddPrecWilsonLinOpFH(state, param.Mass, param.anisoParam);
    }

    //! Return a linear operator solver for this action to solve M*psi=chi
    LinOpSystemSolver<LatticeFermion> *
    EvenOddPrecWilsonFermActFH::invLinOp(Handle<FermState<T, P, Q>> state,
    ...
    EvenOddPrecWilsonFermActFH::invMdagM(Handle<FermState<T, P, Q>> state,
                                         const GroupXML_t &invParam) const
    {
        std::istringstream is(invParam.xml);
        XMLReader paramtop(is);
